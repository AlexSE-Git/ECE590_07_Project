...
def std_train(net, epochs, batch_size, lr, reg, log_every_n=50):
    """
    Training a network
    ...
    for epoch in range(start_epoch, epochs):
        ...
        if val_acc > best_acc:
            best_acc = val_acc
            print("Saving...")
            torch.save(net.state_dict(), "standard.pt")

def NR_train(net, epochs, batch_size, lr, reg, log_every_n=50):
    adv_vec = torch.from_numpy(np.load("adv.npy"))
    lbl_vec = torch.from_numpy(np.load("lbl.npy"))
    ...
    for epoch in range(start_epoch, epochs):
        ...
        for batch_idx, (inputs, targets) in enumerate(trainloader):
            ...
            # Random target label
            t_labels = lbl_vec[batch_idx]
            t_labels = t_labels.to(device)
            
            adv_input = adv_vec[batch_idx]
        ...
        if val_acc > best_acc:
            best_acc = val_acc
            print("Saving...")
            torch.save(net.state_dict(), "NR.pt")
